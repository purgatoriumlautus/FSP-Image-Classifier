...
print(coco.cat_dict.values())  # all classes contained in the dataset
...
fig, ax = plt.subplots(nrows=2, ncols=2, figsize=(15, 10))  # creates a 2×2 grid of subplots to plot four images
ax = ax.ravel()
...
for i, im in enumerate(selected_img_ids):
    ...
    for ann in annotations:
        bbox = ann['bbox']
        x, y, w, h = [int(b) for b in bbox]  # coordinates of bounding box
        class_id = ann["category_id"]
        class_name = coco.load_cats(class_id)[0]["name"]
        license = coco.get_imgLicenses(im)[0]["name"]
        color_ = assign_colour(class_name)
        rect = plt.Rectangle((x, y), w, h, linewidth=2, edgecolor=color_, facecolor='none')  # creates a rectangle based on the extracted coordinates and assigned colour
        t_box = ax[i].text(x, y, class_name, color='red', fontsize=10)
        t_box.set_bbox(dict(boxstyle='square, pad=0', facecolor='white', alpha=0.6, edgecolor='blue'))  # creates box with the name of the category
        ax[i].add_patch(rect)
