...
# GPU Configuration - MUST BE FIRST, BEFORE ANY OTHER TF OPERATIONS
...
if len(physical_devices) > 0:
    try:
        ...
        tf.config.experimental.set_memory_limit(physical_devices[0], 1024 * 10)  # 10GB limit
        print(f"GPU found and configured: {physical_devices[0]}")
    except RuntimeError as e:
        print(f"GPU configuration failed: {e}")
        # Force CPU usage if GPU fails
...
def prepare_datasets():
    ...
    # Fixed batch sizes - don't batch visualization dataset
    train_ds = train_ds.batch(16).prefetch(tf.data.AUTOTUNE)
    val_ds = val_ds.batch(16).prefetch(tf.data.AUTOTUNE)
    vis_ds = vis_ds.prefetch(tf.data.AUTOTUNE)  # Remove .batch(4) - process one at a time
...
def visualize_detections(model, dataset, class_names, num_images=4):
    ...
    dataset_unbatched = dataset.unbatch()  # Remove batching first
    
    count = 0
    for image, targets in dataset_unbatched:
        if count >= num_images:
            break
        ...
        for box, label_name in zip(targets['boxes'], targets['label_names']):
...
def main():
    ...
    try:
        visualize_detections(trained_model, vis_ds, class_names, num_images=2)
    except:
        print("Skipping visualization due to batching issues")
